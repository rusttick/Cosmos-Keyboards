...
- **Accidental-activation risk during a keystroke** — a distinct failure mode neither bullet above catches: finger A's key and finger B's key can each be independently reachable (no blocking violation) and still have a problem, if the act of _pressing_ A's key drags finger B far enough via enslaving to also trigger B's key. This is now computable with real numbers rather than a placeholder, given switch datasheets (pre-travel distance, actuation point) and keycap geometry are available to the algorithm: Step 3's local Jacobian at A's candidate position already gives the position→angle relationship, so `Δθ_i ≈ J⁻¹ · Δtravel` (using the specific switch's pre-travel distance along the press axis) estimates the actual angular excursion of A's keystroke — not just its static reach point, the stroke itself. Apply the measured enslaving coefficient `E[i][j]` to that excursion — `Δθ_j_induced ≈ E[i][j] · Δθ_i` — and compare it against finger B's own margin between its rest angle and its own switch's actuation angle. A candidate placement that would consume too much of that margin risks B's key triggering as a side effect of A's press, not from B moving on its own. **Load-bearing assumption inherited from the capture side:** `E[i][j]` is measured from slow, deliberate exploration (`scan_procedure.md` Phase 5), not validated against real keystroke-speed motion — this check is only as good as that assumption, tracked as an open item there.
...
- **Keycap footprint clearance vs. finger-blocking vs. accidental-activation** stays a three-way distinction, not a two-way one: the exclusion radius in step 2 is pure geometry (cap dimensions); the reachability exclusion in step 4 is kinematic but static (measured joint-angle-_pair_ infeasibility — can both fingers even get there); the accidental-activation check added to step 4 is kinematic and _dynamic_ (does pressing one key's actual stroke drag a neighbor into triggering its own key) — three different mechanisms, all needed, none a substitute for another.
...
| Step 4's accidental-activation check                           | Blocked on switch/keycap geometry (now available) plus `scan_procedure.md`'s `E[i][j]` and ROM data — the one piece carrying an extra untested assumption on top (slow-capture coupling assumed to hold at real keystroke speed) |
